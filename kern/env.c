...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	assert(begin <= end);
	assert(end <= UTOP);
	for (uintptr_t cur = begin; cur < end; cur += PGSIZE)
	{
		struct PageInfo *page = page_alloc(0);
		page_insert(e->env_pgdir, page, (void *)cur, PTE_W | PTE_U);
	}
}
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	struct Proghdr *phs = (void *)(binary + elf->e_phoff);
